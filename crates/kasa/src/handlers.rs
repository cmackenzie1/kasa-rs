...
pub async fn handle_wifi(host: String, port: u16, timeout: Duration, command: WifiCommand) {
    match command {
        WifiCommand::Scan => handle_wifi_scan(&host, port, timeout).await,
        WifiCommand::Join {
            ssid,
            keytype,
            password_stdin,
...
async fn handle_wifi_scan(host: &str, port: u16, timeout: Duration) {
